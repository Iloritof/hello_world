class Task {
  ...
  remainingTime() {
    ...
    if (present < deadline) {
      ...
      if (days > 0) return `${days} days, ${hrs}hrs, ${min} min`;
      else return `${hrs}hrs, ${min} min`;
  ...
  displayTasks(tasklist) {
    ...
    tasklist.forEach((singleTask) => {
      ...
      if (obj.date !== '') {
        ...
        taskDeadline.prepend(toolTip);
      }
      if (obj.date === '') taskDeadline.innerText = `No Deadline Set`;
...
class Controller {
  static initializeApp() {
    ...
    document
      .querySelector('.section-bottom')
      .addEventListener('click', (e) => this.removeNotice(e));
  ...
  static removeNotice(e) {
    const ui = new UI();
    if (e.target.matches('.notice-close')) ui.clearNotify(e.target);
  }
}
